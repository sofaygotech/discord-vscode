...
import { log, LogLevel, showLogs } from './logger';
...
async function sendActivity(sleeping = false) {
	isSleeping = sleeping;
	// eslint-disable-next-line require-atomic-updates
	state = {
		...(await activity(state, isSleeping)),
	};
...
async function login() {
	...
	rpc.on('ready', () => {
		...
		statusBarIcon.tooltip = 'Click for Discord Presence options';
		statusBarIcon.command = 'discord.toggleMenu';
...
export async function activate(context: ExtensionContext) {
	...
	const clearActivity = commands.registerCommand('discord.clearActivity', async () => {
		state = {};
		await rpc.user?.clearActivity();
		await window.showInformationMessage('Cleared Discord Presence activity');
	});

	const toggleMenu = commands.registerCommand('discord.toggleMenu', async () => {
		const items = [
			{ label: '$(sync) Reconnect to Discord', action: 'reconnect' },
			{
				label: config[CONFIG_KEYS.Enabled] ? '$(circle-slash) Disable Presence' : '$(check) Enable Presence',
				action: 'toggle',
			},
			{ label: '$(clear-all) Clear Activity', action: 'clear' },
			{ label: '$(output) Show Logs', action: 'logs' },
		];

		const selected = await window.showQuickPick(items, { placeHolder: 'Discord Presence Options' });
		if (!selected) return;

		if (selected.action === 'reconnect') {
			await disable(false);
			await enable(false);
	...
	const logs = commands.registerCommand('discord.showLogs', () => {
		showLogs();
	});

	context.subscriptions.push(enabler, disabler, reconnecter, disconnect, clearActivity, toggleMenu, logs);
	...
	window.onDidChangeWindowState(async (windowState) => {
		if (windowState.focused) {
			...
			if (isSleeping) {
				await sendActivity(false);
			}
		} else if (config[CONFIG_KEYS.IdleTimeout] !== 0) {
			// eslint-disable-next-line no-restricted-globals
